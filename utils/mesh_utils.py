...
def post_process_mesh(mesh, cluster_to_keep=1000):
    ...
    #删除黑色点U2G-1205
    colors = np.asarray(mesh_1.vertex_colors)
    black_indices = np.where((colors[:, 0] > 0.01) |
                             (colors[:, 1] > 0.01) |
                             (colors[:, 2] > 0.01))[0]
    mesh_0 = mesh_1.select_by_index(black_indices)
...
class GaussianExtractor(object):
    ...
    def reconstruction(self, viewpoint_stack):
        ...
        for i, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="reconstruct radiance fields"):
            ...
            self.uncmaps.append(unc.cpu())
        
        self.rgbmaps = torch.stack(self.rgbmaps, dim=0)
        self.depthmaps = torch.stack(self.depthmaps, dim=0)
        self.alphamaps = torch.stack(self.alphamaps, dim=0)
        self.depth_normals = torch.stack(self.depth_normals, dim=0)
        self.uncmaps = torch.stack(self.uncmaps, dim=0)  #U2G-911
    ...
    def export_image(self, path):
        ...
        for idx, viewpoint_cam in tqdm(enumerate(self.viewpoint_stack), desc="export images"):
            ...
            save_img_u8(self.uncmaps[idx][0].cpu().numpy(), os.path.join(vis_path, 'unc_{0:05d}'.format(idx) + ".png"))